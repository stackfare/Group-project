...
/**
 * Show the Courses in UI in the cart page when they are added to cart.
 */
function displayItemsInCartLayout() {
  ...
  if (cartItems && productContainer) {
    ...
    cartItems.map((item) => {
      productContainer.innerHTML += `
          ...
          <p class="cancelbutton" data-id=${item.name
            .split(" ")
            .join("")} onClick="removeItemFromCartHandler(event)">x</P>
    ...
    if (cartCost == 0) {
      productContainer.innerHTML = `<p><br><br><a href="/mycourses.html" style=color:yellow;>Click Here</a> to view the course(s) you have purchased </p>`;
    }
      ...
      productContainer.innerHTML += `
          <div class="cartTotalContainer">
              <h4 class="cartTotalTitle">
                  <strong><i class='fa fa-shopping-cart' aria-hidden='true'></i>Cart Total:</strong> $${cartCost}.00
              </h4>
              <button class= "Checkout_button" onclick="checkOut()">Buy Course</button>
          </div>
      `;
    }
  }
...
function checkOut() {
  ...
  CheckOutPayment.innerHTML = `
    Total Cost<i class='fa fa-shopping-cart' aria-hidden='true'></i>: $${cartCost}.00
    `;
}
